...
def evaluate(args):
  ...
  if args.save_path:
    ########################################################################
    # 🔴 这里是修改点：关键点更大、更深、更明显
    # 原来：image = draw_image_by_points(args.image, prediction, 1, (255,0,0), False, False)
    # 现在：点大小 = 3，颜色 = 深红色 (0,0,200)，更清晰
    ########################################################################
    image = draw_image_by_points(args.image, prediction, 3, (255, 0, 0), False, False)
